...
public class FrmPedido extends JInternalFrame implements ActionListener, MouseListener {
	...
	private String leerEntidad() {
		...
		if (txtEntidad.getText().trim().length()==0) {
			Tool.mensajeError(this, "Campo entidad esta vac�o !");
			txtEntidad.requestFocus();
		}else if (txtEntidad.getText().trim().matches(Reguex.ENTIDAD_PEDIDO)) {
			res = txtEntidad.getText().trim();
		}else {
			Tool.mensajeError(this, "Entidad inv�lida. Cantidad de caracteres (3 y 20)");
			txtEntidad.requestFocus();
		}
	...
	private String leerRuc() {
		...
		if (txtRuc.getText().trim().length()==0) {
			Tool.mensajeError(this,"Campo del RUC est� vac�o !");
			txtRuc.requestFocus();
		}else if (txtRuc.getText().trim().matches(Reguex.RUC_PEDIDO)) {
			res= txtRuc.getText();
		}else {
			Tool.mensajeError(this,"Ruc inv�lido. Ejemp (XXXXXXXXXXX, 11 digitos)");
			txtRuc.requestFocus();
		}
	...
	private int leerObjeto() {
		...
		if (cboObjeto.getSelectedIndex()==0) {
			Tool.mensajeError(this,"Elige un objeto de contrataci�n");
		}else {
	...
	private String leerFecha() {
		...
		if (dcFecha.getDate()==null) {
			Tool.mensajeError(this, "El campo fecha est� vac�o !");
	...
	private String leerEstado() {
		...
		if (txtEstado.getText().trim().length()==0) {
			Tool.mensajeError(this,"El campo estado est� vac�o !");
	...
	private void limpiar () {
		...
		txtEntidad.setText("MINISTERIO P�BLICO");
